...
public class PanelSalva extends JPanel implements ActionListener {
	/**
	 * 
	 */
	...
	private FrameSalva fs;

	public PanelSalva(FrameSalva fs, GestioneSquadre gs) {
		this.fs = fs;
		this.gs = gs;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (pulsante == btnConferma) {
			if (nomeFile.getText().isEmpty())
				JOptionPane.showMessageDialog(new JFrame(), "Inserisci nome del file", "Errore!",
						JOptionPane.ERROR_MESSAGE);
			else {
				SalvaCaricaSquadre scs = new SalvaCaricaSquadre();
				scs.serializzazione(nomeFile.getText(), gs);
			}
			fs.dispose();
		}
